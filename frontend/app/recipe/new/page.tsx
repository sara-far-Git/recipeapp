...
export default function NewRecipePage() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="card-surface p-5 mb-3 animate-fade-up" style={{ animationDelay: "50ms" }}>
        <div className="flex items-center justify-between gap-4">
          ...
          <button onClick={() => scanInputRef.current?.click()} disabled={scanning}
            className="flex-shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-md btn-fire text-sm font-semibold uppercase tracking-widest">
      ...
      {scanSuccess && (
        <div className="flex items-center gap-2 p-3 mb-3 rounded-xl bg-cinnamon-50 border border-cinnamon-200 text-cinnamon-600 text-sm animate-fade-up">
        ...
        {!importOpen ? (
          <div className="flex items-center justify-between gap-4">
            ...
            <button onClick={() => setImportOpen(true)}
              className="flex-shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-md btn-outline text-sm font-semibold uppercase tracking-widest">
          ...
          <div className="space-y-3">
            ...
            {importError && (
              <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm">{importError}</div>
            )}
            <div className="flex gap-2">
              <button onClick={handleImportFromUrl} disabled={importing || !importUrl.trim()}
                className="flex items-center gap-2 px-5 py-2.5 rounded-md btn-fire text-sm font-semibold disabled:opacity-40">
                {importing ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                ייבוא
              </button>
              <button onClick={() => { setImportOpen(false); setImportUrl(""); setImportError(""); }} disabled={importing}
                className="px-5 py-2.5 rounded-md btn-outline text-sm font-semibold">
                ביטול
              </button>
            </div>
      ...
      {step === 1 && (
        <div className="space-y-6 animate-slide-up opacity-0" style={{ animationFillMode: "forwards" }}>
          ...
          <div>
            ...
            {imageUrl ? (
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={imageUploading}
                className="w-full aspect-video rounded-2xl border-2 border-dashed border-surface-400 bg-surface-100 flex flex-col items-center justify-center gap-2 text-bark-200 hover:border-cinnamon-300 hover:text-cinnamon-500 transition-all duration-300">
                {imageUploading
          ...
          <button onClick={() => setStep(2)} disabled={!title.trim()}
            className="w-full py-3.5 rounded-md btn-fire font-semibold uppercase tracking-widest text-sm flex items-center justify-center gap-2 disabled:opacity-40">
      ...
      {step === 2 && (
        <div className="space-y-4 animate-slide-up opacity-0" style={{ animationFillMode: "forwards" }}>
            ...
            <div key={i} className="flex items-start gap-2 card-surface p-3 animate-fade-up" style={{ animationDelay: `${i * 40}ms` }}>
              ...
              {ingredients.length > 1 && (
                <button onClick={() => removeIngredient(i)}
                  className="p-2 text-bark-100 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors mt-0.5">
          ...
          <button onClick={addIngredient}
            className="w-full py-3 rounded-xl border-2 border-dashed border-surface-400 text-bark-200 hover:border-cinnamon-300 hover:text-cinnamon-500 flex items-center justify-center gap-2 text-sm transition-all duration-300">
            <Plus className="w-4 h-4" /> הוספת מצרך
          </button>

          <div className="flex gap-3">
            <button onClick={() => setStep(1)}
              className="flex-1 py-3 rounded-md btn-outline font-semibold text-sm flex items-center justify-center gap-2">
              <ArrowRight className="w-4 h-4" /> חזרה
            </button>
            <button onClick={() => setStep(3)}
              className="flex-1 py-3 rounded-md btn-fire font-semibold text-sm flex items-center justify-center gap-2">
        ...
        <div className="space-y-4 animate-slide-up opacity-0" style={{ animationFillMode: "forwards" }}>
            ...
            <div key={i} className="flex items-start gap-3 card-surface p-4 animate-fade-up" style={{ animationDelay: `${i * 40}ms` }}>
              ...
              {instructions.length > 1 && (
                <button onClick={() => removeInstruction(i)}
                  className="p-2 text-bark-100 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                  <Trash2 className="w-4 h-4" />
                </button>
              )}
            </div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleSubmit} disabled={submitting || !title.trim()}
              className="flex-1 py-3 rounded-md btn-fire font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-40">

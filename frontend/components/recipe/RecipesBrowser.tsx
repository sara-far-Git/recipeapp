...
export default function RecipesBrowser({ initial }: { initial: CatalogueRecipe[] }) {
  const [recipes, setRecipes] = useState<CatalogueRecipe[]>(initial);
  // Only ever true when the server handed over nothing. With recipes in the
  // props there is no first load to wait for — they are already on the screen
  // and in the HTML, which is the point of fetching them there.
  ...
  // The server reads the catalogue with a short cache and gives up rather than
  // fail the page. When it came back empty — a sleeping backend, most likely —
  // the browser asks again, which is also what the retry button does.
  useEffect(() => {
    if (recipes.length > 0 && attempt === 0) return;
    let cancelled = false;
    async function load() {
      setLoading(true);
      setError(false);
      try {
        const all: CatalogueRecipe[] = [];
  ...
  return <PageFrame tone="forest">
    ...
    {loading && recipes.length === 0 ? <RecipeLoading label="אוסף את המתכונים של הקהילה" /> : error && recipes.length === 0 ? <div role="alert" className="card-surface p-6"><p>לא הצלחנו לטעון את המתכונים.</p><button className="btn-block mt-4" onClick={() => setAttempt(a => a + 1)}>ניסיון נוסף</button></div> : <>

...
export default function Footer() {
  ...
  {canInstall ? (
  <button onClick={handleInstall} className="flex items-center gap-2 px-6 py-2.5  btn-fire text-sm font-semibold transition-all uppercase tracking-widest">
  <Download className="w-4 h-4" />
  {isIOS ? "הוסיפי למסך הבית" : "התקיני כאפליקציה"}
  </button>
  ) : (
  <Link href="/install" className="flex items-center gap-2 px-6 py-2.5  text-sm font-semibold transition-all uppercase tracking-widest text-bark-400 hover:text-cinnamon-500"

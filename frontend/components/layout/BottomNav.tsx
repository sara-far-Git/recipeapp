...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-50 glass-strong border-t border-white/[0.05] sm:hidden">
      <div className="flex items-center justify-around h-16 px-2">
        {items.map((item) => {
          ...
          if (item.special) {
            return (
              <Link key={item.href} href={item.href} className="flex flex-col items-center -mt-5">
                <div className="w-13 h-13 rounded-2xl bg-gradient-to-br from-fire-300 via-fire-500 to-ember-400 flex items-center justify-center shadow-glow active:scale-90 transition-all duration-200" style={{ width: 52, height: 52 }}>
          ...
          return (
            <Link key={item.href} href={item.href} className={cn("flex flex-col items-center gap-1 px-3 py-1.5 transition-all duration-300", active ? "text-fire-300" : "text-smoke-400")}>
